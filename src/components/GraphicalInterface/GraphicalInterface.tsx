...
const TempTerminal: React.FC = () => {
  return (
    <p>Insert terminal as import later!!!</p>
  )
}
const TempSomethingElse: React.FC = () => {
  return (
    <p>Insert something else here!</p>
  )
}
    ...
    >
      ...
      <div className='absolute w-1/2 left-1/2 top-1/2'>
        <DatePicker
          onDatePicked={ date =>{
              setShowWindow(true);
              setWindowContent(<EventDisplay eventDate={date.toISOString().split('T')[0]}/>);
            }
          }
        />
      </div>
